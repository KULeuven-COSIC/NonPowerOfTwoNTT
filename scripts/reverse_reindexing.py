"""
This script is designed to illustrate the reverse reindexing operations on the shuffled result of the
Number-Theoretic Transform (NTT). The script includes functions for reversing the reindexing
caused by Rader's algorithm, Bit Reversal, and PFA, and can be applied to N-dimensional arrays.
The main function, full_reverse_reindexing, combines all these reverse reindexing operations and can
be used to directly obtain the correct ordering of the NTT result with a single permutation.
"""

from scripts.common import root_of_unity
import numpy as np
